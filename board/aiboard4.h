...
if(type == CUT){\
    val = -alphabeta4(self, -alpha-1, -alpha, depth - !mate, CUT, nullmove, true, t1, t2, NULL);\
}else{\
    if(best == -MATE_UPPER){\
        val = -alphabeta4(self, -beta, -alpha, depth - !mate, PV, nullmove, false, t1, t2, NULL);\
    }else{ \
        val = -alphabeta4(self, -alpha-1, -alpha, depth - !mate, CUT, nullmove, true, t1, t2, NULL); \
...
public:
    ...
    AIBoard4()=delete;
    AIBoard4(const char another_state[MAX], bool turn, int round, unsigned char di[2][123], tp* tptable, short** pst, unsigned char** L1, char** _dir) noexcept;
    ...
    virtual bool Think(int maxdepth, unsigned char& src, unsigned char& dst, py::dict* valdict);
