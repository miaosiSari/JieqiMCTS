...
namespace board{
    std::map<std::string, std::function<Thinker*(const char[], bool, int, const unsigned char [2][123], short, std::unordered_map<std::string, bool>*)>> bean; 
    int register_func(std::string x, std::function<Thinker*(const char[], bool, int, const unsigned char [2][123], short score, std::unordered_map<std::string, bool>*)> y){
      bean.insert({x, y});
      return 0;
    }
    int aiboard3 = register_func("AIBoard3", [](const char another_state[], bool turn, int round, const unsigned char di[2][123], short score, std::unordered_map<std::string, bool>* hist)\
 -> Thinker * {return new AIBoard3(another_state, turn, round, di, score, hist);});
    int aiboard4 = register_func("AIBoard4", [](const char another_state[], bool turn, int round, const unsigned char di[2][123], short score, std::unordered_map<std::string, bool>* hist)\
 -> Thinker * {return new AIBoard4(another_state, turn, round, di, score, hist);});
    int aiboard5 = register_func("AIBoard5", [](const char another_state[], bool turn, int round, const unsigned char di[2][123], short score, std::unordered_map<std::string, bool>* hist)\
